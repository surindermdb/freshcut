...
app.prepare().then(async () => {
  ...
  server.use(
    shopifyAuth({
      ...
      async afterAuth(ctx) {
        ...
        console.log("script_tags length", script_tags)

        if (script_tags.length === 0) {
          console.log(HOST , SCRIPT_PATH, '====================================');   
  ...
  if (serverUseSSL) {
    ...
    h2Server.listen(process.env.NODE_PORT || 8081, "0.0.0.0", () => {
      console.log(
        chalk.green(
    ...
    server.listen(process.env.NODE_PORT || 8081,"0.0.0.0", () => {
